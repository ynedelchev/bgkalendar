...
function get() {
  ...
  $periods = array();
  foreach ($gregorianPeriods as $gregorianPeriod) {
    array_push($periods, 
       array(
          'type'  => $gregorianPeriod->getType()->getName(),
          'index' => $gregorianPeriod->getNumber()+1,
          'name'  => $gregorianPeriod->getActualName(),
          'absoluteIndex' => $gregorianPeriod->getAbsoluteNumber()+1,
          'absoluteStartIndex' => $gregorianPeriod->startsAtDaysAfterEpoch()
       )
    );
  }
